...
public class ArticalIndex {
    ...
    public void indexDoc(){
        ...
        if (null != t.getTextblocks())
            doc.add(new TextField("textblock", JSON.toJSONString(t.getTextblocks()), Field.Store.NO));
        if (null != t.getMeshTerms())
            doc.add(new TextField("mesh_term", JSON.toJSONString(t.getMeshTerms()), Field.Store.NO));
